# from https://github.com/ucl-dark/llm_debate/blob/main/core/swiss_tournament.py
    ...
    for round_num in range(num_rounds):
        ...
        if tournament_dir is not None:
            with open(
                ...
                json.dump(scores, f)
        if sorted(player_names, key=lambda p: scores[p]) == [str(y) for y in sorted([int(x) for x in player_names])]:
            return round_num
